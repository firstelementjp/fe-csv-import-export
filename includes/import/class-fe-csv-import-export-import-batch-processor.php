...
class FE_CSV_Import_Export_Import_Batch_Processor extends FE_CSV_Import_Export_Import_Batch_Processor_Base {
	 ...
	 *
	 * @since 0.9.8
	 * @var FE_CSV_Import_Export_Import_Cancel_Manager|null
	 */
	 ...
	 *
	 * @since 0.9.8
	 * @param FE_CSV_Import_Export_Ajax_Import_Batch_Planner|null $batch_planner Batch planner.
	 * @param FE_CSV_Import_Export_Import_Row_Context|null        $row_context_util Row context util.
	 * @param FE_CSV_Import_Export_Import_Meta_Tax|null           $meta_tax_util Meta/tax util.
	 * @param FE_CSV_Import_Export_Import_Persister|null          $persister_util Persister util.
	 * @param FE_CSV_Import_Export_Import_Row_Processor|null      $row_processor_util Row processor util.
	 * @param FE_CSV_Import_Export_Import_Taxonomy_Util|null      $taxonomy_util Taxonomy util.
	 * @param FE_CSV_Import_Export_Import_Cancel_Manager|null     $cancel_manager Import cancel manager.
	 */
	public function __construct(
		?FE_CSV_Import_Export_Ajax_Import_Batch_Planner $batch_planner = null,
		?FE_CSV_Import_Export_Import_Row_Context $row_context_util = null,
		?FE_CSV_Import_Export_Import_Meta_Tax $meta_tax_util = null,
		?FE_CSV_Import_Export_Import_Persister $persister_util = null,
		?FE_CSV_Import_Export_Import_Row_Processor $row_processor_util = null,
		?FE_CSV_Import_Export_Import_Taxonomy_Util $taxonomy_util = null,
		?FE_CSV_Import_Export_Import_Cancel_Manager $cancel_manager = null
	) {
	...
	private function get_batch_planner(): object {
		if ( null === $this->batch_planner ) {
			$this->batch_planner = new FE_CSV_Import_Export_Ajax_Import_Batch_Planner();
		}
	 ...
	 *
	 * @since 0.9.8
	 * @return FE_CSV_Import_Export_Import_Cancel_Manager
	 */
	private function get_cancel_manager(): FE_CSV_Import_Export_Import_Cancel_Manager {
		if ( null === $this->cancel_manager ) {
			$this->cancel_manager = new FE_CSV_Import_Export_Import_Cancel_Manager();
		}
	...
	private function process_batch_import( array $config, array $csv_data, array &$counters ): void {
		...
		if ( isset( $csv_data['batch_lines'] ) && is_array( $csv_data['batch_lines'] ) ) {
			...
			do_action(
				'fe_csv_import_export_preload_relationship_data_for_batch',
				$current_batch_data,
				$csv_data['headers'],
				[
	...
	protected function validate_id_column_or_send_error( array $config, array $csv_data ) {
		...
		if ( 0 === (int) ( $config['start_row'] ?? 0 ) ) {
			...
			if ( ! $validation_result['valid'] ) {
				FE_CSV_Import_Export_Ajax_Util::send_error_response( (string) $validation_result['error'] );
				return false;
			}
	...
	protected function get_row_context_util(): object {
		if ( null === $this->row_context_util ) {
			$this->row_context_util = new FE_CSV_Import_Export_Import_Row_Context( new FE_CSV_Import_Export_Import_Csv() );
		}
	...
	protected function get_meta_tax_util(): object {
		if ( null === $this->meta_tax_util ) {
			$this->meta_tax_util = new FE_CSV_Import_Export_Import_Meta_Tax( null, $this->get_taxonomy_util() );
		}
	...
	protected function get_persister_util(): object {
		if ( null === $this->persister_util ) {
			$this->persister_util = new FE_CSV_Import_Export_Import_Persister();
		}
	...
	protected function get_row_processor_util(): object {
		if ( null === $this->row_processor_util ) {
			$this->row_processor_util = new FE_CSV_Import_Export_Import_Row_Processor();
		}
