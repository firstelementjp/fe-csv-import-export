...
class FE_CSV_Import_Export_Import_Row_Processor {
	...
	public function process_row_context_with_persister(
		...
		array $context,
		array &$counters,
		FE_CSV_Import_Export_Import_Persister $persister,
		$handle_successful_row_import
	): void {
		...
		$this->process_single_import_row_with_persister_without_callbacks(
			$wpdb,
			$is_update,
			$post_id,
	...
	public function process_single_import_row_with_persister_without_callbacks(
		...
		try {
			$persist_result = $persister->persist_post_row_from_csv_with_result(
				$wpdb,
				$is_update,
				$post_id,
				$post_fields_from_csv,
			...
			$this->append_error_detail(
				...
				sprintf(
					...
					__( 'Error: %1$s', 'fe-csv-import-export' ),
					$e->getMessage()
				)
			);
		...
		if ( ! $this->handle_row_result_after_persist_without_callbacks( $persist_result['db_result'], $counters ) ) {
			$this->append_error_detail(
				...
				$is_update,
				$post_id,
				__( 'Error: Failed to save the post.', 'fe-csv-import-export' )
			);
			return;
		}
