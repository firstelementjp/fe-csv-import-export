 ...
 * @subpackage Core
 */
class FE_CSV_Import_Export_License_Handler {
	 ...
	 * @var string
	 */
	public const CRON_HOOK_RESYNC_LICENSE = 'fe_csv_import_export_resync_license';
	...
	public function deactivate( $license_key ) {
		...
		if ( ! defined( 'FE_CSV_IMPORT_EXPORT_LICENSE_API_URL' ) || empty( FE_CSV_IMPORT_EXPORT_LICENSE_API_URL ) ) {
			...
			delete_option( 'fe_csv_import_export_pro_license' );
			self::clear_cache();

			return [
				'success'    => true,
				'message'    => __( 'License deactivated locally. Server-side activation remains active.', 'fe-csv-import-export' ),
				'local_only' => true,
			];
		}
	...
	private function call_license_api( $action, $license_key, $token = '', $product_id = 0 ) {
		...
		$response = wp_remote_post(
			FE_CSV_IMPORT_EXPORT_LICENSE_API_URL,
			[
				'body'    => [
					'action'      => $action,
		...
		if ( json_last_error() !== JSON_ERROR_NONE ) {
			return [
				'success' => false,
				'message' => __( 'Invalid response from license server.', 'fe-csv-import-export' ),
			];
		}
	...
	public static function maybe_decrypt_license_key( $license_key ) {
		...
		if ( ! class_exists( 'FE_CSV_Import_Export_Encryption_Utils' ) || ! FE_CSV_Import_Export_Encryption_Utils::is_available() ) {
			return $license_key;
		}

		$decrypted_key = FE_CSV_Import_Export_Encryption_Utils::decrypt( $license_key );
	...
	public static function prepare_license_key_for_storage( $license_key ) {
		...
		$encrypted_key = FE_CSV_Import_Export_Encryption_Utils::encrypt( $license_key );
	...
	public static function resync_license_from_server() {
		...
		if ( ! ( $result['success'] ?? false ) ) {
			set_transient( 'fe_csv_import_export_pro_license_error', (string) ( $result['message'] ?? '' ), 60 );
		...
		update_option( 'fe_csv_import_export_pro_license', $license_data );
	...
	public static function get_pro_product_id() {
		if ( defined( 'FE_CSV_IMPORT_EXPORT_PRO_PRODUCT_ID' ) && absint( FE_CSV_IMPORT_EXPORT_PRO_PRODUCT_ID ) > 0 ) {
			return absint( FE_CSV_IMPORT_EXPORT_PRO_PRODUCT_ID );
		}
